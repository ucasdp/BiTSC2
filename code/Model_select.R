BIC_fun <- function(X,D,onefolder){
  ...
  for(i in 1:length(rdata)){
    ...
    p <- 2*Params$K+4*Params$M+Params$N+2 #number of paramters: theta(K)+C(N)+pi(1)+Lo(M*2)+Zo(M*2)+tree(K)+rho(1)
    n <- Params$N #number of samples
    bic <- -2*log_like+p*log(n)
    BIC$BIC <- c(BIC$BIC, bic)
  }
  plot(BIC$BIC ~ BIC$K, main = "Model selection", xlab = "K", ylab = "BIC")
